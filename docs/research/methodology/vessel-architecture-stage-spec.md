> **来源**:老库 vessel-aisep/docs/aisep/03_architecture-stage-spec.md。本文为方法论/知识沉淀,非当前 harness 现行强制规范(契约 01:诚实标注建议层)。架构阶段 7 问锚点 + 硬尺寸上限(≤5pp/≤400 LOC),与当前 plan/blast-radius 同源。
