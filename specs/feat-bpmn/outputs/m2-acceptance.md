...
| 14 XSS(M2 面) | 伪造含哨兵 payload 的 `AIFORGE_STATUS`(gate/decision/created_at/feature_id 全塞 payload)→ 项目模式渲染徽章+点节点开详情 | **PASS**(端到端实测,2026-06-12 复验) | `{"pwned":"undefined","injectedNodes":0}`;详情面板字面呈现 `<script>…`/`<img onerror=…>`;同轮顺带二证验收 12(pipeline_xml 缺失走备援源渲染成功) |
...
- ~~验收 14 的"哨兵状态数据"端到端注入未实测~~ → 2026-06-12 T10 评审点名后已补端到端实测(见上表),自报缺口闭环。
